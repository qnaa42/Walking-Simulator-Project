...
namespace Assets.Scripts
{
    public class UserStatsManager : MonoBehaviour
    {
        ...
        public int GetHunger()
        {
            ...
            return _myDataManager.GetHunger(myID);
        }

        public virtual void AddHunger(int anAmount)
        {
            if (!didInit)
                Init();

            _myDataManager.AddHunger(myID, anAmount);
        }

        public virtual void ReduceHunger(int anAmount)
        {
            if (!didInit)
                Init();

            _myDataManager.ReduceHunger(myID, anAmount);
        }

        public virtual void SetHunger(int anAmount)
        {
            if (!didInit)
                Init();

            _myDataManager.SetHunger(myID, anAmount);
        }
    }
}
